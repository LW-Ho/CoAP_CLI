...
class SlotOperation(object):
      ...
      for childid in self.child_dict.keys():
        if cmp(childid.getName(), childKey) is 0:
          ...
          while current_slot_offset > 0 :
            ...
            localqu = NodeInfo.getNodeLQ(childid.getName())
            NodeInfo.updateNodeGQ(childid.getName(), self.nodeKey, localqu*-1) # to delete the queue of numbers.
    ...
    def checkParent(self, parentID):
      ...
      if self.parentID is None :
        ...
        if cmp(parentID.getName(), self.parentID.getName()) is 0:
          ...
          # update parentID
          self.parentID = parentID
          localque = NodeInfo.getNodeLQ(self.getName())
          NodeInfo.updateNodeParent(self.nodeKey, parentID.getName())
          NodeInfo.updateNodeGQ(self.nodeKey, parentID.getName(), localque)
          return 0
    ...
    def checkChild(self, childID):
      if childID not in self.child_dict:
        ...
        if cmp(childID.getChild_numbers(),self.child_dict[childID]) is 1: # not match
          if testing_flag :
            print "childID get Child Numbers : "+str(childID.getChild_numbers())+" and old child numbers : "+str(self.child_dict[childID][0])
